...
-- history_vector_size is also global
	...
	do
		...
		do
			...
			local C3D_vector = func_get_C3D(opt.data_path, opt.class, 1,
											 v, cur_mask[1], cur_mask[2], C3D_m, masked_segs)
			...
			do
				log_file:write('\t\t\tStep: ' .. step_count .. ' ---> Action= ' .. action ..
							...
							' ]; GT = [' .. now_target_gt[1] .. ' , ' .. now_target_gt[2] .. 
							 ' ]; Reward= ' .. reward .. ' ; iou = ' .. new_iou .. '; dist = '
							 .. new_dist .. '\n')
				...
				elseif torch.uniform(torch.Generator()) < epsilon then -- greedy policy
					action = torch.random(torch.Generator(),1,5)
				else
					local tmp_v = 0
				...
				-- 1 move forward; 2 move back; 4 expand; 3 narrow
					...
					old_dist, new_dist, dist_table,old_iou, new_iou, iou_table, index  = 
						func_follow_dist_iou(cur_mask, tmp_gt, available_objects,iou_table,dist_table)
					now_target_gt = tmp_gt[index]
					reward = func_get_reward_movement(old_iou, new_iou,old_dist,new_dist)
					old_iou = new_iou
					old_dist = new_dist
				...
				if action == trigger_action then
					...
					masked = true
					if reward == 3 then
						table.insert(masked_segs, {now_target_gt[1]+torch.floor((now_target_gt[2]-now_target_gt[1]+1)*0.1),
									now_target_gt[2]-torch.floor((now_target_gt[2]-now_target_gt[1]+1)*0.1)})
