...
local mask_rate = 0.1 -- 1-2*mask_rate of current mask will not be used anymore
...
do
	...
	do
		...
		do
			...
			print('\t\tstep ' .. steps .. ' ; iou ' .. iou .. '\n')
												
			local C3D_vector = func_get_C3D(opt.data_path, opt.class, 1,
											 v, mask[1], mask[2], C3D_m, masked_segs)
			...
			if action == trigger_action then
				...
				mask = func_take_action(mask, action, total_frms, act_alpha)
			end
			steps = steps + 1
		end	--while
		lp = lp+1
